...
def KNN(video_url, num_comments=1000):
    ...
    # Ensure sensitivity_covariate is a string
    sensitivity_covariate = str(sensitivity_covariate)
...
if youtube_link:
    try:
        ...
        # Display Word Cloud for KNN Comments
        comments_knn = comments_df_LR['Comment']
        st.subheader("Word Cloud for Comments")
        create_wordcloud(comments_knn)

        # Percentage of Positive, Negative, and Neutral Comments (using LR)
        ...
        with st.sidebar:
            ...
            st.write(f"True positive rate: {true_positive_rate_lr:.2f}")
            st.write(f"False positive rate: {false_positive_rate_lr:.2f}")
            st.write(f"Predicted Sentiment with Covariate Change: {sensitivity_prediction_lr[0]} ")
            st.write(f"Confusion matrix: {confusion_lr}")
            st.write(f"Classification report: {report_lr}")

            st.markdown("""-----------------""")
            st.write("K-Nearest Neighbours for text analytics is a popular choice but due to its inability to handle "
                     "sparse text data, returns a lesser accuracy")
            st.subheader("METRICS FOR K-Nearest Neighbours:")
            ...
            st.write(f"Classification report: {report_knn}")

        labels = ['Positive', 'Negative', 'Neutral']
        sizes = [positive_percentage_lr, negative_percentage_lr, neutral_percentage_lr]
